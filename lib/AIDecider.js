...
1. Är batteriet under golv-nivån: välj de N NÄRMASTE fönstren — nödsituation, pris spelar ingen roll.
2. Är det BRÅTTOM med resa (slotsKvar <= N + 2): välj de N NÄRMASTE fönstren — bilen måste nå målet, pris spelar ingen roll.
3. Annars: välj de N BILLIGASTE tillgängliga fönstren.
...
class AIDecider {
  ...
  async plan({ currentBattery, floor, normalTarget, nextTrip, currentSlot, avg5day, futurePrices, chargerKw = 11, batteryKwh = 82, slotsNeeded: n }) {
    ...
    if (nextTrip) {
      ...
      const latestStart = new Date(nextTrip.departureTime.getTime() - n * 15 * 60000);
      const mustChargeNow = now >= latestStart;
      tripStr = `Avgång ${toSE(nextTrip.departureTime)} (om ${hoursUntil}h), mål ${nextTrip.targetPercent}%\n` +
        `  Behöver ${n} slots (${n * 15} min laddtid) | Senaste starttid: ${toSE(latestStart)}` +
        (mustChargeNow ? ' — MÅSTE LADDA NU, ingen tid kvar att vänta!' : ` (${Math.round((latestStart - now) / 60000)} min kvar att optimera)`);
    }

    const taperNote = currentTarget > 80
      ? `OBS: AC-laddning taprar vid högt batteri — 80–90% går på 65% effekt, 90–100% på 40%. Därför är N högre än väntat.\n`
      : '';
