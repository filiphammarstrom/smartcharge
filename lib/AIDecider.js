...
class AIDecider {
  ...
  async decide({ currentBattery, floor, normalTarget, nextTrip, currentSlot, avg5day, futurePrices, chargerKw = 11, batteryKwh = 75 }) {
    ...
    // When a trip is active, only show slots before departure (the rest are irrelevant)
    const slots = nextTrip
      ? (futurePrices || []).filter(p => new Date(p.time_start) < nextTrip.departureTime)
      : (futurePrices || []).slice();

    const slotsAvailBeforeDep = nextTrip
      ? Math.floor((nextTrip.departureTime - now) / (15 * 60000))
      : null;
    ...
    // Highlight cheapest upcoming slots (all before departure if trip is active)
    ...
    const depNote = nextTrip && slotsAvailBeforeDep !== null
      ? `\nOBS: Alla priser ovan är FÖRE avgångstiden ${toSE(nextTrip.departureTime)}. ` +
        `Det finns ${slotsAvailBeforeDep} slots (à 15 min) kvar innan avgång. ` +
        `Slots efter avgångstiden är INTE tillgängliga och visas inte här.`
      : '';
    ...
    const userMessage =
      ...
      `Nästa resa: ${tripStr}\n\n` +
      `De 8 billigaste tillgängliga slottarna:\n${cheapLines || '(ingen data)'}` +
      `${depNote}\n\n` +
      `Alla tillgängliga priser (${slots.length} slots, datum/tid → SEK/kWh):\n` +
