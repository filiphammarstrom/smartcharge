...
/**
 * Charging rate per 15-minute slot based on charger power, battery capacity,
 * and AC charging taper above 80 %.
 *
 * Taper model (typical Tesla AC charging):
 *   0–80 %  → full rate
 *   80–90 % → 65 % of full rate
 *   90–100% → 40 % of full rate
 */
function slotChargePercent(batteryLevel, chargerKw = 11, batteryKwh = 75) {
 ...
 */
function slotsNeeded(from, to, chargerKw = 11, batteryKwh = 75) {
  let slots = 0;
  let level = from;
...
class Scheduler {
  ...
  constructor(opts) {
    ...
    this.batteryKwh = opts.batteryKwh ?? 75;
  }
